...
const SENSITIVITIES = [
  ...
  { value: 'scalp_sensitivity', label: 'Scalp Sensitivity', desc: 'Sensitive scalp, prone to irritation' },
]
...
export default function QuestionnairePage() {
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {POROSITY.map((t) => (
                  <motion.button type="button" key={t.value}
                    ...
                    className="flex flex-col items-center gap-2 p-3 rounded-lg border cursor-pointer text-center"
                    style={{
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <motion.span key={formData.grayPercent} initial={{ scale: 1.3, color: '#9333EA' }} animate={{ scale: 1, color: '#9333EA' }} className="font-bold text-lg text-[#9333EA]">
                  {formData.grayPercent}%
                </motion.span>
              </div>
              <input type="range" min={0} max={100} step={5} value={formData.grayPercent}
                onChange={(e) => updateField('grayPercent', Number(e.target.value))}
                className="w-full h-2 rounded-full appearance-none cursor-pointer bg-white/10"
                style={{ background: `linear-gradient(to right, #9333EA 0%, #9333EA ${formData.grayPercent}%, rgba(255,255,255,0.1) ${formData.grayPercent}%, rgba(255,255,255,0.1) 100%)` }}
              />
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {SENSITIVITIES.map((c) => (
                  <motion.button type="button" key={c.value}
          ...
          <div className="space-y-5">
            <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                <div><span className="text-white/40">Name:</span>{' '}<span className="text-[#F5F5F7] font-medium">{formData.clientName || '—'}</span></div>
                <div><span className="text-white/40">Phone:</span>{' '}<span className="text-[#F5F5F7] font-medium">{formData.phone || '—'}</span></div>
                <div><span className="text-white/40">Email:</span>{' '}<span className="text-[#F5F5F7] font-medium">{formData.email || '—'}</span></div>
              </div>
              {formData.salonNotes && (
                <div className="text-sm"><span className="text-white/40">Salon Notes:</span><p className="text-[#F5F5F7]/70 mt-1">{formData.salonNotes}</p></div>
              )}
            </div>
            ...
            <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                <div><span className="text-white/40">Texture:</span>{' '}<span className="text-[#F5F5F7] font-medium">{TEXTURES.find(t => t.value === formData.texture)?.label || '—'}</span></div>
                <div><span className="text-white/40">Pattern:</span>{' '}<span className="text-[#F5F5F7] font-medium">{HAIR_PATTERNS.find(t => t.value === formData.hairPattern)?.label || '—'}</span></div>
                <div><span className="text-white/40">Density:</span>{' '}<span className="text-[#F5F5F7] font-medium">{DENSITIES.find(t => t.value === formData.density)?.label || '—'}</span></div>
                <div><span className="text-white/40">Porosity:</span>{' '}<span className="text-[#F5F5F7] font-medium">{POROSITY.find(t => t.value === formData.porosity)?.label || '—'}</span></div>
                <div><span className="text-white/40">Gray:</span>{' '}<span className="text-[#F5F5F7] font-medium">{formData.grayPercent}%</span></div>
              </div>
              {formData.scalpType && (
                <div className="text-sm"><span className="text-white/40">Scalp:</span>{' '}<span className="text-[#F5F5F7] font-medium">{SCALP_TYPES.find(t => t.value === formData.scalpType)?.label}</span></div>
              )}
                ...
                <div className="text-sm">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {formData.sensitivities.map((c) => (
                      <span key={c} className="px-2 py-1 rounded-full text-xs bg-yellow-500/15 text-yellow-400 border border-yellow-500/20">
  ...
  return (
    <motion.div className="min-h-screen bg-[#0F0F0F] text-[#F5F5F7] p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      <div className="max-w-2xl mx-auto">
        <motion.div className="mb-8 text-center" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-[#9333EA] to-[#0D9488] bg-clip-text text-transparent">New Consultation</h1>
        ...
        <GlassCard className="p-6" hover={false}>
          <motion.div className="mb-4" key={`title-${step}`} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        ...
        <motion.div className="flex items-center justify-between mt-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
          <Button variant="outline" onClick={handleBack} disabled={step === 1} className="border-white/10 text-[#F5F5F7] hover:bg-white/5 hover:text-[#F5F5F7] disabled:opacity-30">
            ← Back
          </Button>

          {step < totalSteps ? (
            <Button onClick={handleNext} disabled={!canProceed()} className="bg-[#9333EA] hover:bg-[#9333EA]/90 text-white disabled:opacity-30">
              Next →
            </Button>
          ) : (
            <div className="flex gap-3">
              <Button onClick={handleSaveClient} disabled={saving} variant="outline" className="border-white/10 text-[#F5F5F7] hover:bg-white/5 hover:text-[#F5F5F7]">
                {saving ? <Loader2 size={16} className="animate-spin mr-2" /> : null}
                Save Client
              </Button>
              <Button onClick={handleSaveAndFormulate} disabled={saving} className="bg-gradient-to-r from-[#9333EA] to-[#0D9488] hover:opacity-90 text-white font-semibold">
                {saving ? <Loader2 size={16} className="animate-spin mr-2" /> : null}
                Save & Formulate →
              </Button>
            </div>
