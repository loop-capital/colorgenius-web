...
import type { Tone, HairConditionType, Porosity } from '../types';
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          <ChipSelector
            ...
            options={HAIR_TYPES}
            selected={hairType}
            onSelect={(v) => setHairType(v as HairConditionType)}
          />
          <ChipSelector
            label="Porosity"
            options={POROSITY}
            selected={porosity}
            onSelect={(v) => setPorosity(v as Porosity)}
          />
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {BRANDS.map((b) => (
                  ...
                  >
                    <Text style={[chipStyles.chipText, brandPref === b && chipStyles.chipTextActive]}>
                      {b}
                    </Text>
                  </TouchableOpacity>
                ))}
          ...
          {result && (
            <View style={styles.resultCard}>
              ...
              {result.developerVolume && (
                <Text style={styles.resultMeta}>Developer: {result.developerVolume}</Text>
              )}
              {result.processingTime && (
                <Text style={styles.resultMeta}>Process: {result.processingTime}</Text>
              )}
              {result.application && (
                <Text style={styles.resultMeta}>Application: {result.application}</Text>
              )}
